...
export (bool) var marked = false

func _ready():
	._ready()
	
	state_variables.append_array(["tag", "marked"])
...
func disable():
	...
	if marked:
		self.get_owner().spawn_mark(self.get_pos())
...
func spawn_object(projectile:PackedScene, pos_x:int, pos_y:int, relative = true, data = null, local = true):
	...
	self.get_owner().recursive_style_modulation(obj)
