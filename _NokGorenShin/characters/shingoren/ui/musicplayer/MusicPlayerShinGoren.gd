...
func _on_Tray_toggled(button_pressed):
	if button_pressed == true:
		$"%Tray".icon = load("res://_NokGorenShin/characters/shingoren/ui/musicplayer/musicplayer-traySHOW.png")
		$"%MusicVBoxContainer".visible = true
	
	else:
		$"%Tray".icon = load("res://_NokGorenShin/characters/shingoren/ui/musicplayer/musicplayer-trayHIDE.png")
...
func _ready():
	...
	if host.fighter:
		...
		if host.fighter.get_node("%Stuff").skin == "UberOni":
...
func _process(delta):
	if host.fighter:
		...
		#	--	CONDITIONS
		$"%MusicPlayer".visible = host.fighter.get_node("%Stuff").music_access == true and host.fighter.get_node("%Stuff").skin != null
		play_music = host.fighter.game_over != true
		#play_music = (host.fighter.terminus == true) and host.fighter.game_over != true
		...
		if songs:
			...
			if play_music == true:
				...
				if host.fighter.opponent.combo_count >= 1 or "MusicVolumeQuiet" in host.fighter.current_state().get("editor_description"):
					$"%MusicAudioPlayer".volume_db = lerp($"%MusicAudioPlayer".volume_db, volumes.Losing, 1)
				
				elif host.fighter.combo_count >= 1 or "MusicVolumeLoud" in host.fighter.current_state().get("editor_description"):
			...
			if music_player_data.muted == true:
				$"%Mute".icon = load("res://_NokGorenShin/characters/shingoren/ui/musicplayer/musicplayer-musicOFF.png")
			else:
				$"%Mute".icon = load("res://_NokGorenShin/characters/shingoren/ui/musicplayer/musicplayer-musicON.png")
