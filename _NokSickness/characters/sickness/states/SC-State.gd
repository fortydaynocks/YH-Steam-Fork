extends CharacterState

#	--	DRAG SCRIPT
export var _c_drag = 0
export (bool) var drag = false
export (int) var offset_x = 0
export (int) var offset_y = 0
export (int) var start_on = 1
export (int) var end_on = 1
export (float) var drag_strength = 2.5
export (bool) var force_drag = false

export (int) var _c_Sickness
export (int) var hit_illness = 0
export (int) var block_illness = 0
export (bool) var auto_block_illness = true
#export (int) var glac_block = 50

#	========================================================================== |
func _on_hit_something(obj, hitbox):
	._on_hit_something(obj, hitbox)
	
	if obj == host.opponent:
		if hit_illness > 0:
			host.increment_property(host.illness, hit_illness)

func on_got_blocked_by(who):
	.on_got_blocked_by(who)
	
	if who == host.opponent:
		if auto_block_illness:
			host.increment_property(host.illness, host.opponent.blocked_hitbox_plus_frames + block_illness)
			
		else:
			host.increment_property(host.illness, block_illness)

#	========================================================================== |
func _enter():
	._enter()
	
	
func _tick():
	._tick()
	
	if (hit_fighter == true and drag == true) or (force_drag == true):
		if (current_tick < end_on) and (current_tick > start_on):
			var pos = host.get_pos()
			var opos = host.opponent.get_pos()
			
			host.opponent.set_vel(0, 0)
			host.opponent.move_directly(str((pos.x + (offset_x * host.get_facing_int()) - opos.x) / drag_strength), str(((pos.y + offset_y) - opos.y) / drag_strength))
